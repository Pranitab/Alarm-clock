function alarmClock(){
	...
	function checkAlarm(){
	  ...
	  {
		  //dayArr=[" Mon"," Tue"," Wed"," Thr"," Fri"," Sat"," Sun"];
		  ...
		  if(alarmData[i].snoozeVal == "")
		  {
			...
			{
				for(var j=1;j<alarmData[i].note.split(",").length;j++)
				{
					console.log(alarmData[i].note.split(",")[j],today.toString().split(" ")[0]);
					console.log(alarmData[i].note.split(","),today.toString().split(" ")[0]);
					if(alarmData[i].note.split(",")[j]==(" "+today.toString().split(" ")[0]))
					{
						//console.log("tadaaaaaaaaaaaa");
						playAlarmNo = i;
						if(alarmData[playAlarmNo].enable)
						{
		  ...
		  //console.log("aaa",today.toString().split(" ")[0]);
			...
			for(i in radioElement)
			{
				radioElement[i].onclick = function(e){
					if(e.target.getAttribute("clicked") == "false")
					{
						...
						e.target.style.backgroundColor = "#000fff";
						e.target.setAttribute("clicked",true);
						console.log(element,alarmData[element].ringTone,e.target.getAttribute("ringNo"));
						alarmData[element].ringTone = Number(e.target.getAttribute("ringNo"));
					}
					else
					{
						for(var j=0;j<radioElement.length;j++)
	...
	function changeScreen(btn,ele){
		switch(btn) {
		  ...
		  case "playAlarm":
			currScreen = "screen_5"
			//console.log(alarmData[playAlarmNo].enable);
			document.getElementById("midPanel").innerHTML = "";
			playAlarm();
	...
	function playAlarm(){
		...
		//srcElement = document.getElementsByTagName("source");
		//console.log(srcElement);
		//srcElement.setAttribute("src","./audios/2.mp3");
		audio = document.getElementById("myAudio");
		audio.src="./audios/"+alarmData[playAlarmNo].ringTone+".mp3"
		audio.play(); 
		stopAlarm();
	}
	function snoozeAlarm(){
		var snoozeBtn = document.getElementsByClassName("snoozeBtnComp")[0];
				snoozeBtn.onclick = function(e){
					...
					if(alarmData[playAlarmNo].snoozeVal == "")
					{
						var min = Number(alarmData[playAlarmNo].minute)+1;
					...
					{
						var min = Number(alarmData[playAlarmNo].snoozeVal.split(":")[1])+1;
						var ampm = alarmData[playAlarmNo].ampm;
						if(min>=60)
						{
							var hr = Number(alarmData[playAlarmNo].hour)+1;
	...
	function addAlarm(){
		...
		var soundArr = ["Morning Melody","Radar","Beep"]
		for(var i=0; i< soundArr.length; i++)
		{
			...
			radioComp.setAttribute("ringNo", i);
			dayComp.append(radioComp);
			console.log("alarmdata",alarmData,currentAlarm);
		}
	...
	function editAlarm(element){
		...
		for(var j=1;j<alarmData[eleNo].note.split(",").length;j++)
			...
			console.log(alarmData[eleNo].ringTone);
		document.getElementsByClassName("radioComp")[alarmData[eleNo].ringTone].style.backgroundColor = "#000fff";
		document.getElementsByClassName("radioComp")[alarmData[eleNo].ringTone].setAttribute("clicked",true);
		
		addRadioEvents(eleNo);
	}
				...
				saveElement.onclick = function(){
					...
					else
					{
						alarmData[currentAlarm]={
								...
								"note":noteEle+dayEle,
								"enable":true,
								"snooze":alarmData[currentAlarm].snooze,
								"ringTone":alarmData[currentAlarm].ringTone,
								"snoozeVal":""
							};
					}
